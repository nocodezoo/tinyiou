...
export default function CreateIOU() {
  ...
  // Handle pre-filled recipient from Discover page
  useEffect(() => {
    const handleParams = () => {
      ...
      if (username && !receiverProfile) {
        setTimeout(async () => {
          const { data } = await supabase.from('profiles').select('id, username').ilike('username', username.replace('@', '')).single()
          if (data) {
            setInput({ ...input, receiver: username.replace('@', '') })
  ...
  const handleInviteSubmit = async () => {
    ...
    try {
      ...
      const { data, error: fnError } = await supabase.functions.invoke('send-invite', {
        body: { email: inviteEmail, invite_link: inviteLink }
      })
      
      if (fnError) {
        console.log('Email may not have sent:', fnError)
        // Still show success since invite is saved
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        type: 'iou_received',
        title: 'New IOU Received',
        body: `${(user as any).username} honored you with an IOU`,
        link: `/profile/${(user as any).username}`
      })
      navigate('/')
    } catch (err: any) { 
            ...
            {!inviteSent ? (
              <>
                <label className="flex items-center gap-2 mb-3 cursor-pointer">
                  <input type="checkbox" checked={inviteMode} onChange={e => { setInviteMode(e.target.checked); setInput({...input, receiver: ''}); }} className="w-4 h-4 accent-orange-500" />
                  <span className="text-sm text-zinc-400">Invite someone new via email</span>
                </label>
                
                {inviteMode ? (
                  <div className="space-y-3">
                    <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)}
                      className="w-full bg-zinc-900 border-2 border-zinc-700 rounded-xl py-3 px-4 text-lg font-bold text-white placeholder-zinc-500 focus:border-orange-500 outline-none" placeholder="email@example.com" />
                    <button onClick={handleInviteSubmit} disabled={!inviteEmail || loading} className="w-full mt-3 bg-orange-500 text-white py-2.5 rounded-xl font-bold transition disabled:opacity-40 shadow-[0_0_20px_rgba(249,115,22,0.4)]">
              ...
              <div className="text-center py-4">
                ...
                <p className="text-white font-bold mb-2">Invite sent!</p>
                <p className="text-zinc-500 text-sm mb-4">We&apos;ve sent an invitation to {inviteEmail}</p>
                <button onClick={() => { setInviteSent(false); setInviteMode(false); setInviteEmail(''); }} className="text-orange-400 text-sm font-bold">Send another invite</button>
              </div>
            )}
          </motion.div>
        )}
        ...
        {step === 2 && (
          <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-zinc-800 rounded-3xl p-5 shadow-xl">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap gap-1">{SPARK_SUGGESTIONS[input.taxonomy]?.map(s => <button key={s} onClick={() => setInput({ ...input, message: s })} className="px-2 py-1 bg-zinc-700 hover:bg-orange-500/50 text-[10px] font-medium text-zinc-300 hover:text-white rounded-full transition-all">{s}</button>)}</div>
        ...
        {step === 3 && (
          <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-zinc-800 rounded-3xl p-4 shadow-xl">
            ...
            <div className="flex gap-2 mb-3">
              <label className="flex-1 py-2 text-xs font-bold uppercase bg-zinc-900 text-white rounded-xl text-center cursor-pointer hover:bg-zinc-700 transition"><Image className="w-4 h-4 inline mr-1" /> Gallery<input ref={fileInputRef} type="file" accept="image/*,video/*" className="hidden" onChange={handleImageUpload} /></label>
