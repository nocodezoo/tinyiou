...
export default function MindMap({ profileId, currentUsername }: MindMapProps) {
  ...
  const loadConnections = async () => {
    try {
      // Get IOUs given by this user - explicitly select avatar_url
      const { data: given } = await supabase
        .from('ious')
        .select('creator_id, receiver:receiver_id(id, username, avatar_url)')
        .eq('creator_id', profileId)
        .limit(50)

      // Get IOUs received by this user
      const { data: received } = await supabase
        .from('ious')
        .select('receiver_id, creator:creator_id(id, username, avatar_url)')
        ...
        if (iou.receiver && iou.receiver.username !== currentUsername) {
          ...
          if (existing) {
            ...
            connMap.set(iou.receiver.username, {
              ...
              avatar_url: iou.receiver.avatar_url || null,
              type: 'given',
              count: 1
            })
          }
      ...
      // Process received IOUs
      ;(received || []).forEach((iou: any) => {
        if (iou.creator && iou.creator.username !== currentUsername) {
          ...
          if (existing) {
            ...
            connMap.set(iou.creator.username, {
              ...
              avatar_url: iou.creator.avatar_url || null,
              type: 'received',
              count: 1
            })
          }
  ...
  const getFallbackAvatar = (username: string) => {
    return `https://api.dicebear.com/7.x/identicon/svg?seed=${username}&backgroundColor=0a0a0b`
  }
  ...
  return (
    ...
    >
      ...
      {connections.map((conn, i) => {
        ...
        const avatarSrc = conn.avatar_url || getFallbackAvatar(conn.username)
              ...
              <div className="w-full h-full rounded-full bg-zinc-900 flex items-center justify-center overflow-hidden">
                <img 
                  ...
                  onError={(e) => {
                    ...
                    target.parentElement!.innerHTML = `<span class="text-white font-bold text-xs">${conn.username[0].toUpperCase()}</span>`
                  }}
                />
              </div>
            </Link>
